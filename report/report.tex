...
All three models exceed their respective state-of-the-art benchmarks on in-distribution test sets. EfficientNet-B4 achieves 99.83\% accuracy and 99.56\% F1 on HistoricalCrack18-19 (target: 95\%). YOLOv8l achieves 70.3\% mAP@50 on the OmniCrack30k validation set (target: 70\%, exceeded) via 3-phase progressive resolution training (320\,px\,$\to$\,640\,px\,$\to$\,1024\,px), with test mAP@50 of 55.2\% on OmniCrack30k rising to 63.01\% after heritage domain fine-tuning. MAnet with MiT-B2 encoder achieves 97.5\% Crack IoU on the combined OmniCrack30k, Masonry, and CrackForest test split (target: 80\%), with sigmoid output calibration optimised for deployment (threshold 0.8 inverted).
...
Test set (standard)       & 55.2\%          & 30.0\%             & 54.6\%             & 69.0\%          & 97 \\
...
The validation mAP@50 of 70.3\% meets the 70\% SotA target. Test mAP@50 reaches 55.2\% under standard inference and 63.0\% with TTA, demonstrating that the phase-trained model generalises to the held-out test partition. The 15 pp validation-to-test gap is substantially smaller than the old single-phase baseline, reflecting the benefit of progressive resolution training on crack localisation. Cross-domain performance on heritage masonry (CrackForest, Masonry subsets) is evaluated separately in Section~\ref{sec:domain_adapt}.
...
\begin{table}[H]
\centering
\caption{MAnet + MiT-B2 segmentation results on the combined OmniCrack30k + Masonry + CrackForest test split. The target Crack IoU of 80\% is exceeded by 17.5 pp. Deployment uses threshold 0.8 (inverted: values $< 0.8$ indicate cracks).}
...
\midrule
Crack IoU (threshold 0.8 inverted) & \textbf{97.5\%} & Above 80\% & Target exceeded \checkmark \\
Dice coefficient (crack class) & 98.1\% & Secondary metric & Exceptionally high \\
...
OmniCrack30k test & In-distribution & mAP@50 & 55.22\% \\
...
\item \textbf{Segmentation (MAnet / MiT-B2):} 97.5\% Crack IoU and 98.1\% Dice on the combined OmniCrack30k, Masonry, and CrackForest test split, exceeding the 80\% target by 17.5 pp. Vision transformer backbones with multi-scale attention gates enable superior morphological capture. Deployment uses threshold 0.8 with inverted sigmoid logic, a domain-specific calibration discovered during web application validation.
...
In this project, YOLOv8l is the detection backbone for the OmniCrack30k single-class model (reaching 63.01\% mAP@50 on the test split via 3-phase progressive training) and YOLOv8s is used for the DACL10k multi-class damage detector. The anchor-free design reduces the manual effort of configuring anchor boxes for the diverse aspect ratios of crack bounding boxes, and the model variants balance accuracy with inference speed suitable for a web application.
