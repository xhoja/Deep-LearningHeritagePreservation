...
\begin{abstract}
\noindent
This paper presents a multi-task deep learning pipeline for automated structural damage assessment of cultural heritage buildings, motivated by the preservation needs of UNESCO-listed sites in Albania including Berat, Gjirokastr, and Butrint. The system chains three computer vision tasks in a sequential pipeline: binary damage classification using EfficientNet-B4, crack spatial localisation via YOLOv8s object detection, and pixel-level crack mask prediction using a U-Net with ResNet34 encoder. The central research question is \textit{cross-dataset generalisation}: whether models trained on large-scale industrial crack datasets (road surfaces, concrete pavement) can transfer their learned representations to the visually distinct domain of historic stone masonry and plaster facades.

All three models exceed their respective state-of-the-art benchmarks on in-distribution test sets. EfficientNet-B4 achieves 99.83\% accuracy and 99.56\% F1 on HistoricalCrack18-19 (target: 95\%). YOLOv8s reaches 96.7\% mAP@50 on OmniCrack30k validation (target: 70\%), though a pronounced cross-domain gap emerges on the held-out test split (34.6\%). U-Net achieves 83.56\% mIoU and 81.26\% Dice on the combined Masonry and CrackForest test split (target: 80\%).
...
\item \textbf{Task 2 (Detection):} Crack bounding-box localisation using YOLOv8s trained on OmniCrack30k ($\approx$30,000 multi-surface crack images). Cross-domain evaluation and heritage-specific domain adaptation are both investigated.

\item \textbf{Task 3 (Segmentation):} Pixel-level crack mask prediction using U-Net with ResNet34 encoder, trained on a combined heritage dataset of 358 annotated image-mask pairs from two sources.
...
[Task 1]  Damage Classification  (EfficientNet-B4)
     ...
     |
     v  cracked
[Task 2]  Crack Detection  (YOLOv8s)
          Output: bounding boxes + confidence scores
     |
     v
[Task 3]  Crack Segmentation  (U-Net + ResNet34)
...
\subsubsection{Architecture: YOLOv8s}

YOLOv8s~\cite{ultralytics2023} is the small variant of the YOLOv8 detection family. The architecture follows the YOLO paradigm of single-stage detection: a backbone extracts multi-scale features, a feature pyramid neck aggregates context across scales, and a detection head predicts bounding boxes, objectness, and class probabilities in a single forward pass. YOLOv8s has 11.2M parameters and operates at approximately 50 FPS on a T4 GPU at image size 640, making it well-suited for real-time crack localisation.
...
\subsubsection{Architecture: U-Net with ResNet34 Encoder}

The segmentation model uses a U-Net~\cite{ronneberger2015unet} decoder paired with a ResNet34~\cite{he2016resnet} encoder initialised from ImageNet pre-trained weights, implemented via the \texttt{segmentation-models-pytorch} library. ResNet34 has 4 residual stages with output feature map sizes proportional to the input (stride 2 at each stage), producing features at $1/2$, $1/4$, $1/8$, and $1/16$ of the input resolution.

The U-Net decoder reconstructs the full-resolution segmentation map through four upsampling stages, each concatenating the corresponding encoder skip-connection features with the upsampled decoder features. Final output is a $384 \times 384$ binary map (crack / background) obtained by applying a sigmoid activation to the single output channel and thresholding at 0.5.
...
\begin{table}[H]
\centering
\caption{YOLOv8s detection results on OmniCrack30k. The large validation-to-test gap (62.1 pp) directly demonstrates cross-domain generalisation degradation.}
...
\rowcolors{2}{lightgray}{white}
\begin{tabular}{p{3.5cm}p{3.0cm}p{2.5cm}cc}
\toprule
\rowcolor{white}
\textbf{Model} & \textbf{Test Domain} & \textbf{Split Type} & \textbf{Metric} & \textbf{Value} \\
\midrule
\multirow{3}{3.5cm}{Classifier (EfficientNet-B4)} & HistoricalCrack & In-distribution & Recall & 99.83\% \\
 & CrackForest & Out-of-distribution & Recall & 89.83\% \\
 & Masonry & Out-of-distribution & Recall & 76.67\% \\
\midrule
\multirow{2}{3.5cm}{Segmentor (U-Net / ResNet34)} & CrackForest subset & In-distribution & mIoU & 73.61\% \\
 & Masonry subset & In-distribution & mIoU & 86.92\% \\
\midrule
\multirow{3}{3.5cm}{Detector (YOLOv8s)} & OmniCrack30k test & In-distribution & mAP@50 & 34.59\% \\
...
\begin{table}[H]
\centering
\caption{YOLOv8s detection failure analysis on the HistoricalCrack18-19 test split (780 images) at confidence threshold 0.25. Recall of 1.000 confirms the detector never misses an actual crack.}
\label{tab:det_fail}
\rowcolors{2}{lightgray}{white}
\begin{tabular}{lcc}
\toprule
...
The hypothesis is that SAHI would improve heritage crack detection because hairline cracks are thin and potentially sub-resolution at 640px. The fine-tuned YOLOv8s checkpoint (Task 5) is used. Configuration: adaptive slice size 256--512 px, 20\% overlap, confidence threshold 0.25.
...
\midrule
Plain YOLOv8s (imgsz=640) & \textbf{28.0\%} & Normal rate & Full-image inference \\
YOLOv8s + SAHI (256--512 tiles) & 18.9\% & Fewer (0.59 per image) & Sliced tile inference \\
\midrule
...
Training configuration: YOLOv8s, \texttt{freeze=10} backbone layers frozen, \texttt{lr0=0.005}, batch=16, imgsz=640, 50 epochs with patience=15. Training time approximately 3.5 hours on T4 GPU.
...
All model inference runs concurrently using a \texttt{ThreadPoolExecutor} with \texttt{max\_workers=4}. The four concurrent tasks are: (1) EfficientNet-B4 classification and Grad-CAM map; (2) YOLOv8s detection; (3) U-Net segmentation; (4) surface texture analysis (Gabor filter bank and Laplacian complexity map). This design ensures that total response time is approximately equal to the slowest individual model rather than the sum of all four, providing a practical interactive experience.
...
\item \textbf{Detection card:} Displays YOLOv8s bounding boxes overlaid on the input image. A blue badge indicates when a preprocessing filter has been applied before inference.
...
\item \textbf{Detection (YOLOv8s):} 96.7\% mAP@50 on OmniCrack30k validation, exceeding the 70\% target. A 62.1 pp validation-to-test gap (96.7\% to 34.6\%) directly demonstrates cross-domain generalisation degradation. Heritage-specific fine-tuning on 250 images recovers 23.6\% mAP on the combined heritage test set, a 168\% relative improvement.
...
\midrule
Architecture & EfficientNet-B4 & YOLOv8s & U-Net / ResNet34 \\
...
\newblock In \textit{International Conference on Medical Image Computing and Computer-Assisted Intervention (MICCAI)}, pp. 234--241.
...
\newblock In \textit{Proceedings of the European Conference on Computer Vision (ECCV)}, pp. 801--818.
...
\newblock \textit{IEEE Transactions on Pattern Analysis and Machine Intelligence}, 39(6), pp. 1137--1149.

\bibitem{he2016resnet}
He, K., Zhang, X., Ren, S., and Sun, J. (2016).
\newblock Deep Residual Learning for Image Recognition.
\newblock In \textit{Proceedings of the IEEE Conference on Computer Vision and Pattern Recognition (CVPR)}, pp. 770--778.

\bibitem{tan2019efficientnet}
Tan, M. and Le, Q. V. (2019).
\newblock EfficientNet: Rethinking Model Scaling for Convolutional Neural Networks.
\newblock In \textit{Proceedings of the International Conference on Machine Learning (ICML)}, pp. 6105--6114.
...
\newblock \textit{International Journal of Computer Vision}, 128(2), pp. 336--359.

\bibitem{dais2021}
Dais, D., Bal, I. E., Smyrou, E., and Sarhosis, V. (2021).
\newblock Automatic crack classification and segmentation on masonry surfaces using convolutional neural networks and transfer learning.
\newblock \textit{Automation in Construction}, 125, 103606.

\bibitem{mishra2024}
Mishra, M. and Loureno, P. B. (2024).
\newblock Deep learning and computer vision for damage detection in cultural heritage structures: A review.
\newblock \textit{Journal of Cultural Heritage}, 65, pp. 275--292.

\bibitem{redmon2016yolo}
Redmon, J., Divvala, S., Girshick, R., and Farhadi, A. (2016).
\newblock You Only Look Once: Unified, Real-Time Object Detection.
\newblock In \textit{Proceedings of the IEEE Conference on Computer Vision and Pattern Recognition (CVPR)}, pp. 779--788.
...
\bibitem{omnicrack}
Benz, C. and Rodehorst, V. (2022).
\newblock OmniCrack30k: A benchmark for crack segmentation and the multi-task learning of cracks.
\newblock \textit{arXiv preprint arXiv:2203.07033}.

\bibitem{dacl10k}
Benz, C., Flotzinger, J., and Rodehorst, V. (2023).
\newblock DACL10k: Benchmark for semantic bridge damage segmentation.
\newblock In \textit{Proceedings of the IEEE/CVF Winter Conference on Applications of Computer Vision (WACV)}.

\bibitem{lin2017focal}
Lin, T.-Y., Goyal, P., Girshick, R., He, K., and Dollar, P. (2017).
\newblock Focal loss for dense object detection.
\newblock In \textit{Proceedings of the IEEE International Conference on Computer Vision (ICCV)}, pp. 2980--2988.
