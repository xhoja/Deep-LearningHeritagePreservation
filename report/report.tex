...
\begin{table}[H]
\centering
\caption{YOLOv8l (3-phase) detection results on OmniCrack30k. Validation uses Phase 2 best checkpoint; standard test uses the 97-image training subset; TTA is evaluated on the full 581-image OmniCrack30k test split in notebook 05.}
...
Test set (standard, 97-image subset)       & 50.3\%          & 30.0\%             & 54.6\%             & 69.0\%          & 97 \\
Test set (TTA, full OmniCrack30k test)     & \textbf{63.0\%} & ---                & ---                & ---             & 581 \\
SotA published target     & 70.0\%          & No target          & No target          & No target       & n/a \\
\bottomrule
\end{tabular}
\end{table}

The validation mAP@50 of 70.3\% meets the 70\% SotA target. On the 97-image training test subset, standard inference achieves 50.3\% mAP@50. TTA evaluation on the full 581-image OmniCrack30k test split (notebook 05) achieves 63.0\%, demonstrating consistent gains from test-time augmentation across both evaluation scales. The 20 pp validation-to-standard-test gap reflects distributional shift between the OmniCrack30k validation and test partitions. Cross-domain performance on heritage masonry (CrackForest, Masonry subsets) is evaluated separately in Section~\ref{sec:domain_adapt}.
...
CrackForest subset & In-distribution (subset)$^\dagger$ & mIoU & 73.61\% \\
Masonry subset & In-distribution (subset)$^\dagger$ & mIoU & 86.92\% \\
Segmentor --- Masonry+CrackForest (cross-domain, OmniCrack weights) & Out-of-distribution & mIoU & 59.49\% \\
\midrule
...
\bottomrule
\end{tabular}
\par\vspace{2pt}\footnotesize{$^\dagger$ CrackForest and Masonry were included in segmentor training. These figures reflect within-distribution subset performance, not cross-domain generalisation.}
\end{table}
...
\textbf{Segmentor generalisation.} Note that CrackForest and Masonry were both included in the segmentor's training set; the figures in Table~\ref{tab:cross_eval} are within-distribution subset performance, not cross-domain generalisation. Within-subset mIoU is 73.61\% (CrackForest; crack IoU: 48.78\%, Dice: 65.58\%) and 86.92\% (Masonry; crack IoU: 75.01\%, Dice: 85.72\%). The 13.31 pp gap reflects morphological difficulty: Masonry cracks are wider and higher-contrast than the thin, irregular CrackForest road cracks. The true cross-domain result -- evaluating the OmniCrack30k-pretrained segmentor on the combined Masonry+CrackForest test set -- is 59.49\% mIoU (Crack IoU: 35.24\%, Background IoU: 83.75\%, Dice: 44.85\%), a substantial drop that reflects the domain gap between pavement and heritage building surfaces.
...
\begin{table}[H]
\centering
\caption{YOLOv8l detection failure analysis on the HistoricalCrack18-19 test split (780 images) at confidence threshold 0.25. Precision 0.652 reflects selective firing; recall 0.296 reflects the OmniCrack30k-to-HistoricalCrack domain gap.}
\label{tab:det_fail}
\rowcolors{2}{lightgray}{white}
\begin{tabular}{lcc}
\toprule
...
\midrule
True Positives & 45 & Correctly detected cracks \\
False Positives & 24 & Intact textures incorrectly flagged \\
False Negatives & \textbf{107} & \textbf{Actual cracks missed by detector} \\
Precision & \textbf{0.652} & Selective: detector does not over-fire on heritage textures \\
Recall & 0.296 & Low: domain gap causes majority of cracks to be missed \\
\bottomrule
\end{tabular}
\end{table}

The upgraded YOLOv8l detector is far more conservative than the YOLOv8s baseline: it fires selectively, achieving precision 0.652, because it does not respond indiscriminately to all heritage textures. However, recall is only 0.296 -- the detector misses 107 of 152 cracked images. This is a domain adaptation failure in the opposite direction: OmniCrack30k training images are pavement and concrete photographs, and the features learned for those surfaces do not transfer to HistoricalCrack plaster and stone facade textures. The critical failure mode is therefore Type B (silent misses), not Type A (false alarms).

The chained pipeline architecture partially compensates. The upstream classifier (99.49\% test accuracy) gates the detector and rejects intact images before they reach the detection stage, reducing the false-positive load. However, the low detector recall means the pipeline will miss cracks that the classifier correctly flags as cracked -- a direct consequence of the OmniCrack30k-to-HistoricalCrack domain gap that motivates the domain adaptation experiments in Section~\ref{sec:domain_adapt}.
...
\begin{table}[H]
\centering
\caption{Cross-task consistency analysis on HistoricalCrack18-19 test split (780 images). Type B = 111 is the critical failure mode: the detector silently misses cracks the classifier flags, a direct consequence of the OmniCrack30k-to-HistoricalCrack domain gap.}
...
\midrule
Type A: classifier=intact, detector fires (FP load) & 24 & Classifier gate working well: intact images rarely reach detector \\
Type B: classifier=cracked, detector silent (missed crack) & \textbf{111} & \textbf{Critical: detector misses cracks the classifier flags} \\
Classifier-detector agreement on cracked images & 645 / 780 & 82.7\% overall agreement \\
\bottomrule
\end{tabular}
\end{table}

Type A drops from the YOLOv8s baseline value of 624 to 24, confirming that the classifier gate is working well: intact images are rarely forwarded to the detector. However, Type B = 111 is the critical failure mode: the detector silently misses 111 cracked images that the classifier correctly flags. This is a direct consequence of the OmniCrack30k-to-HistoricalCrack domain gap -- the detector has not learned features that transfer to building facade textures. Overall classifier-detector agreement is 645/780 (82.7\%). This consistency gap motivates the domain adaptation experiments in Section~\ref{sec:domain_adapt}, which target the detector recall deficit.
...
\item The \textbf{segmentor} generalises moderately to genuinely out-of-distribution data. Within-subset (in-distribution) mIoU is 73.61--86.92\% on CrackForest and Masonry, both of which were in the training set. The true cross-domain result (OmniCrack30k-pretrained weights on combined Masonry+CrackForest) is 59.49\% mIoU, a 14--27 pp drop that nonetheless indicates partial transfer. The encoder-decoder architecture with skip connections preserves spatial information that supports crack-structure generalisation across surface types.
...
The sequential classifier-detector-segmentor pipeline provides a natural compensation mechanism for the detector's domain gap. The classifier, which generalises well, acts as a domain-level filter: its 99.49\% accuracy on intact images means that the vast majority of intact test images are routed away from the detector before it can generate false alarms (Type A cases drop from 624 to 24 when using the upgraded YOLOv8l). The dominant remaining failure mode is Type B: 111 cracked images are missed by the detector even after the classifier correctly flags them, which the pipeline cannot compensate for without improving detector recall on heritage surfaces.
