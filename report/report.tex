...
Across all six datasets the pipeline uses 57,291 annotated images (Table~\ref{tab:datasets_full}).
Several characteristics identified during exploration directly motivated design decisions.
...
OmniCrack30k        & Segmentation / Detection & 22,158 & 3,277 & 4,582 & 30,017 \\
\midrule
...
\textbf{Class imbalance in DACL10k.}
Parsing all 6,935 YOLO label files reveals a pronounced long-tail class distribution (Figure~\ref{fig:dacl_class_dist}).
The \textit{crack} class accounts for the majority of annotations, while \textit{wetspot} has roughly one third as many instances.
This imbalance motivates the use of focal loss during YOLOv8l training and class-specific confidence thresholds during evaluation.

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{../outputs/eda/04_detection_stats.png}
\caption{DACL10k training set: class distribution (left), normalised bounding-box width/height scatter coloured by class (centre), and annotations-per-image histogram with mean indicated (right). The class imbalance and the concentration of small bounding boxes (width/height $<$ 0.1) inform both the loss function choice and the use of SAHI tiled inference.}
\label{fig:dacl_class_dist}
\end{figure}

\textbf{Crack pixel sparsity.}
Computing the crack pixel density $\rho = \frac{1}{HW}\sum_{i,j}\mathbf{1}[M(i,j)=1]$ for every mask in the
segmentation datasets reveals a strongly right-skewed distribution: CrackForest has a median $\rho = 1.31\%$
and Masonry a median $\rho \approx 3\%$.
This extreme class imbalance at pixel level (up to 99:1 background-to-crack ratio) is the primary reason pixel
accuracy is a misleading metric — a model predicting all pixels as background achieves $>$98\% accuracy yet
Intersection-over-Union of zero.
Tversky loss with $\alpha = 0.7$ (penalising false negatives more than false positives) directly addresses this.

\begin{figure}[H]
\centering
\begin{subfigure}[b]{0.48\textwidth}
\includegraphics[width=\textwidth]{../outputs/eda/07_segmentation_gallery.png}
\caption{Segmentation gallery: original image, ground-truth binary mask, and red-channel overlay. $\rho$ values annotated on each mask.}
\label{fig:seg_gallery}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.48\textwidth}
\includegraphics[width=\textwidth]{../outputs/eda/08_crack_density.png}
\caption{Crack pixel density distribution across CrackForest, Masonry, and OmniCrack30k (300-image sample). All three datasets are right-skewed, confirming that IoU is the appropriate evaluation metric.}
\label{fig:crack_density}
\end{subfigure}
\caption{Segmentation dataset analysis.}
\end{figure}

\textbf{Spatial annotation distribution.}
Aggregating bounding-box centres from all 20,971 DACL10k training annotations into a $80\times80$ heatmap
(Gaussian-smoothed, $\sigma=2$) shows a near-uniform spatial distribution with a mild tendency towards the
image centre-bottom.
No strong positional bias was found, indicating that the dataset does not trivially reward spatial priors.

\textbf{Domain shift.}
RGB channel histograms reveal that DACL10k and CrackForest images are greyer and have lower saturation than
StructDamage (concrete vs.\ general building photography).
This motivates the use of ImageNet normalisation
$\hat{x}_c = (x_c - \mu_c)/\sigma_c$ with $\boldsymbol{\mu}=(0.485, 0.456, 0.406)$ and
$\boldsymbol{\sigma}=(0.229, 0.224, 0.225)$, which maps each dataset's pixel distribution closer to the
ImageNet statistics assumed by the pre-trained backbone weights.
To quantify this gap numerically, Figure~\ref{fig:brightness_contrast} plots violin distributions of mean
greyscale intensity (brightness) and pixel standard deviation (contrast) across all six datasets.
CrackForest images are consistently darker than the ImageNet neutral grey ($\mu=128$), while StructDamage
is brighter.
Contrast is highest for CrackForest and lowest for Masonry, confirming that hairline cracks in Masonry
are harder to separate from background texture on the basis of intensity contrast alone.
These measurements directly informed the colour jitter augmentation parameters (brightness $\pm 0.3$,
contrast $\pm 0.3$) used throughout training.

\begin{figure}[H]
\centering
\includegraphics[width=0.96\textwidth]{../outputs/eda/10b_brightness_contrast.png}
\caption{Domain shift quantified via brightness (mean greyscale intensity) and contrast (pixel std dev)
across all six datasets. Each violin shows the full distribution over 120 sampled images; the white bar
spans the interquartile range. Datasets below the neutral grey line (128) will produce systematically
different activations in an ImageNet-pretrained backbone, motivating colour jitter augmentation.}
\label{fig:brightness_contrast}
\end{figure}

\textbf{Defect class co-occurrence.}
Beyond marginal class counts, the co-occurrence structure of DACL10k annotations is informative.
Counting how many training images contain each pair of defect classes and row-normalising yields a
conditional probability matrix $P(\text{col class} \mid \text{row class present})$
(Figure~\ref{fig:cooccurrence}).
Crack is present in a large fraction of images annotated with other damage types (efflorescence, spalling),
while wetspot tends to appear as a more isolated finding.
These co-occurrence patterns motivate joint multi-class detection rather than five independent binary
classifiers; they also explain a source of hard negatives where two visually similar damage types
(e.g.\ crack vs.\ efflorescence edges) co-exist in the same bounding box region.

\begin{figure}[H]
\centering
\includegraphics[width=0.96\textwidth]{../outputs/eda/04b_cooccurrence.png}
\caption{DACL10k defect class co-occurrence matrix. Left: raw image counts per class pair (diagonal =
total images containing that class). Right: conditional probability
$P(\text{col class} \mid \text{row class present})$, row-normalised.
High off-diagonal values indicate classes that reliably co-occur, motivating joint multi-label detection.}
\label{fig:cooccurrence}
\end{figure}

\textbf{Bounding-box size morphology.}
The scatter plot of normalised bounding-box dimensions (Figure~\ref{fig:dacl_class_dist}) suggests
class-specific size clusters, but the full distribution shape is better characterised by violin plots
(Figure~\ref{fig:bbox_violin}).
Crack boxes are narrow (low width, broad height) consistent with elongated fractures; weathering and
efflorescence produce large, diffuse boxes covering broad surface areas; wetspot boxes cluster near
zero in both dimensions, indicating small localised patches.
These scale differences directly inform the observed class-specific mAP ordering
(weathering highest at 22.7\%, wetspot lowest at 6.6\%) and motivate the use of SAHI tiling for small
wetspot and crack instances.

\begin{figure}[H]
\centering
\includegraphics[width=0.96\textwidth]{../outputs/eda/04c_bbox_violin.png}
\caption{Per-class bounding-box width (left) and height (right) distributions as violin plots.
White horizontal line = median; thick white bar = interquartile range.
Crack boxes are narrow and elongated; weathering boxes are large and diffuse;
wetspot boxes are systematically small.}
\label{fig:bbox_violin}
\end{figure}

Per-class spatial density maps (Figure~\ref{fig:perclass_heatmaps}) decompose the aggregate
annotation heatmap into five independent views.
Weathering and crack show near-uniform spatial distributions across the image plane,
while efflorescence and wetspot exhibit mild clustering in the lower-centre region,
consistent with gravity-driven salt crystallisation and moisture accumulation at facade bases.
No class shows a strong positional bias that would trivially reward spatial anchoring strategies.

\begin{figure}[H]
\centering
\includegraphics[width=0.98\textwidth]{../outputs/eda/05b_perclass_heatmaps.png}
\caption{Per-class annotation density maps (Gaussian-smoothed, $\sigma=2.5$, $60\times60$ grid)
for each of the five DACL10k defect classes.
Each map is independently normalised; $\bigstar$ marks the densest grid cell.
Colour scales use class-matched colourmaps for visual distinctiveness.}
\label{fig:perclass_heatmaps}
\end{figure}

\textbf{Crack morphology.}
Crack pixel density $\rho$ characterises the aggregate crack burden per image, but the
\textit{structure} of cracks --- how many disconnected segments exist, and how large each is ---
is equally relevant to segmentation difficulty.
Connected-component labelling of every ground-truth binary mask (Figure~\ref{fig:crack_morphology})
reveals that CrackForest images contain multiple small, disconnected crack segments (hairline cracking
network), while Masonry images contain fewer but larger connected components.
This structural difference predicts that false-negative risk is higher on CrackForest (where many
thin branches must each be detected), while geometric precision matters more on Masonry (where
large region boundaries must be traced accurately).

\begin{figure}[H]
\centering
\includegraphics[width=0.98\textwidth]{../outputs/eda/08b_crack_morphology.png}
\caption{Crack morphology analysis via connected-component labelling. Left: log-scale distribution of
crack segment area (px$^2$) per dataset; dashed lines indicate class means. Centre: number of distinct
crack segments per image. Right: box-plot comparison of log segment area.
CrackForest has more, smaller segments; Masonry has fewer, larger ones.}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.62\linewidth]{../outputs/tsne_efficientnet_b4.png}
\caption{t-SNE of EfficientNet-B4 average-pool features (1792-dim) across three domains. HistoricalCrack (green circles), CrackForest (orange triangles), and Masonry (red squares) form three clearly separated clusters, confirming genuine domain shift. The partial overlap between HistoricalCrack and CrackForest is consistent with the 9.97 pp recall drop; the larger Masonry separation explains the 23.16 pp drop.}
\label{fig:tsne}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.72\linewidth]{../outputs/dacl10k_detector/train/labels.jpg}
\caption{DACL10k v2 training split annotation statistics. Top-left: per-class instance counts (spalling dominates at 8,484 boxes; wetspot is least represented at 1,443). Top-right: bounding box spatial distribution (most boxes cluster centrally). Bottom: box centre and size distributions, showing the majority of damage annotations are small-to-medium sized and spatially centred, consistent with close-range bridge inspection photography.}
\label{fig:dacl_labels}
\end{figure}
